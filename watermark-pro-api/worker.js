  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === '/health') {
        ...
        if (env.DB) {
          try { await env.DB.prepare('SELECT 1 AS ok').first(); database = 'ok'; }
          catch (e) { console.error('health_db_error', e?.stack || e); database = 'error'; }
        }
...
function isMutation(method) { return !['GET', 'HEAD', 'OPTIONS'].includes(method); }
...
function parseOrigins(env) {
  const configured = String(env.ALLOWED_ORIGINS || 'https://socmegy.com,https://www.socmegy.com,https://control.socmegy.com,https://socmegy.github.io,http://localhost:4173,http://127.0.0.1:4173')
    .split(',').map(v => v.trim()).filter(Boolean);
  // Development/Codex file preview sends Origin: null. Keep production origins and allow that preview origin too.
...
function authToken(request) { return bearerToken(request) || parseCookie(request, SESSION_COOKIE); }
...
function publicUser(row) {
  return {
    ...
    plan: row.plan,
    status: row.status,
    publicProfile: row.public_profile === undefined ? true : Boolean(row.public_profile),
...
async function registerUser(request, env, cors) {
  ...
  await env.DB.batch([env.DB.prepare(`INSERT INTO users (id,username,email,password_hash,password_salt,role,photo,plan,status,created_at,downloads_images,downloads_videos)
    VALUES (?,?,?,?,?,'user','','free','active',?,0,0)`)
    .bind(id, username, email, hash, salt, nowIso()),
...
async function changePassword(request, env, cors, user) {
  ...
  const token = await createSession(env, user.id); const headers = new Headers(cors); setSessionCookie(headers, token);
...
async function updateProfile(request, env, cors, user) {
  ...
  const publicProfile = body.publicProfile === undefined ? (user.public_profile === undefined ? true : Boolean(user.public_profile)) : Boolean(body.publicProfile);
  if (usernameBody.length < 2 || usernameBody.length > 20) throw new HttpError(400, 'Nama pengguna mestilah 2 hingga 20 aksara.');
  if (!validEmail(email)) throw new HttpError(400, 'E-mel tidak sah.');
...
async function addDownloadUsage(request, env, cors, user) {
  const body = await readBody(request);
  const images = clampInt(body.images, 0, 1000, 0);
  const videos = clampInt(body.videos, 0, 1000, 0);
  await env.DB.prepare('UPDATE users SET downloads_images=downloads_images+?, downloads_videos=downloads_videos+? WHERE id=?')
    .bind(images, videos, user.id).run();
  const updated = await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();
...
async function submitPayment(request, env, cors, user) {
  ...
  const submission = {
    id: uid('sub'), userId: user.id, username: user.username, email: user.email,
    planName: plan.name, reference, amount: Number(plan.discount_price ?? plan.normal_price ?? 0),
    paymentTime, submittedAt: nowIso(), status: 'PENDING'
  };
  await env.DB.batch([env.DB.prepare(`INSERT INTO submissions (id,user_id,username,email,plan_name,reference,amount,payment_time,submitted_at,status)
    VALUES (?,?,?,?,?,?,?,?,?,'PENDING')`)
    .bind(submission.id, submission.userId, submission.username, submission.email, submission.planName, submission.reference, submission.amount, submission.paymentTime, submission.submittedAt),
...
function automaticNotification(env, audience, title, text, eventId) {
  return env.DB.prepare("INSERT INTO notifications (id,title,text,audience,sender_username,sender_photo,created_at) VALUES (?,?,?,?,COALESCE((SELECT admin_username FROM settings WHERE id=1),'@watermarkpro'),COALESCE((SELECT admin_photo FROM settings WHERE id=1),''),?)")
    .bind('notice-'+eventId, title, text, normalizeUsername(audience), nowIso());
}
...
async function getPublicState(env) {
  ...
  return {
    ...
    topUsers: topRows.map(r => ({
      id: r.id, username: r.username, photo: String(r.photo || '').trim(), plan: r.plan, status: r.status, publicProfile: r.public_profile === undefined ? true : Boolean(r.public_profile),
      createdAt: r.created_at, proUntil: r.pro_until,
...
async function getAccountState(env, user) {
  const [submissionsRes, subsRes, noticesRes] = await env.DB.batch([
    ...
    env.DB.prepare('SELECT * FROM notifications WHERE audience=\'ALL\' OR lower(audience)=lower(?) ORDER BY created_at DESC LIMIT 100').bind(user.username)
  ]);
  return {
...
function planFromRow(r) {
  let features = [];
  try { features = JSON.parse(r.features_json || '[]'); } catch {}
  return {
    id: r.id, slug: r.slug, name: r.name, normalPrice: Number(r.normal_price || 0),
    discountPrice: Number(r.discount_price || 0), discountLabel: r.discount_label || '',
    billingLabel: r.billing_label || '', description: r.description || '', features,
    active: Boolean(r.active)
  };
}
function submissionFromRow(r) {
  return { id: r.id, userId: r.user_id, username: r.username, email: r.email || '', planName: r.plan_name,
    reference: r.reference, amount: Number(r.amount || 0), paymentTime: r.payment_time || '-', submittedAt: r.submitted_at, status: r.status };
}
function subscriptionFromRow(r) {
  return { id: r.id, userId: r.user_id, username: r.username, planName: r.plan_name, reference: r.reference || '',
    receiptNo: r.receipt_no || '', amount: Number(r.amount || 0), months: Number(r.months || 1), startsAt: r.starts_at,
    endsAt: r.ends_at, createdAt: r.created_at, paymentTime: r.payment_time || '-', status: r.status };
}
function noticeFromRow(r) {
  return { id: r.id, title: r.title, text: r.text, audience: r.audience, senderUsername: r.sender_username,
    senderPhoto: r.sender_photo || '', createdAt: r.created_at };
}
...
async function saveSettings(request, env, cors) {
  ...
  if (communityUrl) {
    let parsed;try { parsed=new URL(communityUrl); } catch { throw new HttpError(400,'Pautan Telegram tidak sah.'); }
  ...
  const authBanners = bannerListFromValue(settings.authBanners === undefined ? current?.auth_banners : settings.authBanners);
  const overviewBanners = bannerListFromValue(settings.overviewBanners === undefined ? current?.overview_banners : settings.overviewBanners);
  try {
    await env.DB.prepare(`UPDATE settings SET admin_username=?,admin_photo=?,payments_open=?,qr_image=?,whatsapp=?,auth_banners=?,overview_banners=?,community_url=? WHERE id=1`)
      .bind(
        username,
        photo,
...
async function createControlUser(request, env, cors) {
  ...
  const id = uid('user');
  await env.DB.batch([env.DB.prepare(`INSERT INTO users (id,username,email,password_hash,password_salt,role,photo,plan,status,created_at,pro_until,downloads_images,downloads_videos)
    VALUES (?,?,?,?,?,'user',?,?,?,?,?,?,?)`)
    .bind(id, username, email, hash, salt, String(body.photo || ''), body.plan === 'pro' ? 'pro' : 'free', body.status === 'inactive' ? 'inactive' : 'active', nowIso(), null, Number(body.downloads?.images || 0), Number(body.downloads?.videos || 0)),
    automaticNotification(env, username, 'Akaun dicipta', 'Akaun Watermark Pro anda berjaya dicipta.', 'registered-'+id)]);
...
async function updateControlUser(request, env, cors, id) {
  ...
  if (duplicate) throw new HttpError(409, 'Nama pengguna atau e-mel sudah digunakan.');
  await env.DB.batch([
    env.DB.prepare(`UPDATE users SET username=?,email=?,photo=?,plan=?,status=?,downloads_images=?,downloads_videos=? WHERE id=?`)
      .bind(username, email, String(body.photo || ''), body.plan === 'pro' ? 'pro' : 'free', body.status === 'inactive' ? 'inactive' : 'active', Number(body.downloads?.images || 0), Number(body.downloads?.videos || 0), id),
...
async function grantPro(request, env, cors, id) {
  ...
  const fallbackStart = user.plan === 'pro' && user.pro_until && new Date(user.pro_until) > new Date() ? user.pro_until : nowIso();
  ...
  await env.DB.prepare(`INSERT INTO subscriptions (id,user_id,username,plan_name,reference,receipt_no,amount,months,starts_at,ends_at,created_at,payment_time,status)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(uid('history'), id, user.username, 'Penyokong Pro', ref, `WP-${ref}`, 0, months, start, end, createdAt, '-', 'GRANTED').run();
  return json({ ok: true, state: await getControlState(env) }, 200, cors);
}
...
async function removePro(env, cors, id) {
  ...
  await env.DB.prepare(`INSERT INTO subscriptions (id,user_id,username,plan_name,reference,receipt_no,amount,months,starts_at,ends_at,created_at,payment_time,status)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(uid('history'), id, user.username, 'Penyokong Pro', ref, `WP-${ref}`, 0, 0, null, createdAt, createdAt, '-', 'REMOVED').run();
  return json({ ok: true, state: await getControlState(env) }, 200, cors);
}
...
async function reviewPayment(request, env, cors, id) {
  ...
  const start = user.plan === 'pro' && user.pro_until && new Date(user.pro_until) > new Date() ? user.pro_until : createdAt;
...
async function saveNotice(request, env, cors, id) {
  ...
  const settings = await env.DB.prepare('SELECT * FROM settings WHERE id=1').first();
  if (id) {
    await env.DB.prepare('UPDATE notifications SET title=?,text=?,audience=?,sender_username=?,sender_photo=? WHERE id=?')
      .bind(title, text, audience, settings?.admin_username || '@watermarkpro', settings?.admin_photo || '', id).run();
  } else {
    await env.DB.prepare('INSERT INTO notifications (id,title,text,audience,sender_username,sender_photo,created_at) VALUES (?,?,?,?,?,?,?)')
      .bind(uid('notice'), title, text, audience, settings?.admin_username || '@watermarkpro', settings?.admin_photo || '', nowIso()).run();
  }
...
async function saveLegal(request, env, cors) {
  ...
  for (const type of ['terms', 'privacy']) {
    ...
    (Array.isArray(doc.sections) ? doc.sections : []).forEach((section, index) => {
      statements.push(env.DB.prepare('INSERT INTO legal_sections (id,type,title,text,position,updated_at) VALUES (?,?,?,?,?,?)')
        .bind(String(section.id || uid(type)), type, String(section.title || '').trim(), String(section.text || '').trim(), (index + 1) * 10, updatedAt));
    });
  }
  await env.DB.batch(statements);
  return json({ ok: true, state: await getControlState(env) }, 200, cors);
...
async function handleSetupPost(request, env) {
  ...
  await env.DB.prepare(`INSERT INTO users (id,username,email,password_hash,password_salt,role,photo,plan,status,created_at,downloads_images,downloads_videos)
    VALUES (?,?,?,?,?,'admin','','pro','active',?,0,0)`)
    .bind(uid('admin'), username, email, hash, salt, nowIso()).run();
