(()=>{
 ...
 window.WPStableAvatar=(node,user)=>{
  ...
  const fallback=()=>{const span=document.createElement('span');span.className='wp-avatar-letter';span.textContent=letter;node.replaceChildren(span)};
  ...
  image.onerror=()=>{if(node.dataset.stableAvatar!==key)return;node.__wpAvatarPending=null;node.dataset.stableAvatarFailed=key;if(!node.querySelector('img'))fallback()};
  image.src=src;
 };
 let finishing=false;
