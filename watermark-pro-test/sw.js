const CACHE='watermark-pro-shell-v147';
const BASE=new URL('./',self.location.href).pathname;
const SHELL=['','index.html','control.html','manifest.webmanifest','logo.jpg','bg.jpg','pwa.js','wmark-shared.js','release-ui.js','control-release.js','icon-192.png','icon-512.png'].map(p=>BASE+p);
...
self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('watermark-pro-shell-')&&k!==CACHE).map(k=>caches.delete(k))))));
...
self.addEventListener('fetch',event=>{
 ...
 const isPage=req.mode==='navigate'&&['','index.html','profil','sokongan','tetapan'].includes(url.pathname.slice(BASE.length).replace(/\/$/,''));
