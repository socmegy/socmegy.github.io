...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#wpInstallApp').count(),1,'install/shortcut button remains available before browser prompt');
  ...
  assert.equal(await page.locator('#wpInstallHelp[open]').count(),1,'click opens the two-action chooser');
  assert.equal(await page.locator('#wpInstallNative').count(),1,'chooser offers direct app installation');
  assert.equal(await page.locator('#wpAddShortcut').count(),1,'chooser offers direct shortcut creation');
  await page.locator('#wpInstallNative').click();
  assert.equal(await page.evaluate(()=>promptCalls),1,'Install App action invokes the native prompt');
