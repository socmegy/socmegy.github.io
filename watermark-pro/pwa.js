(()=>{
  ...
  const ownStandalone=()=>sessionStorage.getItem(launchKey)==='watermark-pro'&&(matchMedia('(display-mode: standalone)').matches||navigator.standalone===true);
  const state={prompt:null,installed:ownStandalone()};
  ...
  matchMedia('(display-mode: standalone)').addEventListener('change',()=>{state.installed=ownStandalone();if(state.installed)state.prompt=null;notify()});
