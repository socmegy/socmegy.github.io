...
(async()=>{
 ...
 try{
  ...
  const own=JSON.parse(fs.readFileSync('manifest.webmanifest'));const rootAppsDir=path.join(__dirname,'archive','publish-root-apps');for(const name of ['admintest','adminnntesttt']){const other=JSON.parse(fs.readFileSync(path.join(rootAppsDir,name+'.webmanifest')));assert.notEqual(other.id,own.id);assert(!own.start_url.startsWith(other.scope));assert(!other.start_url.startsWith(own.scope));}
