...
(async()=>{
 ...
 try{
  ...
  const manifest=await page.evaluate(()=>fetch(new URL('manifest.webmanifest',document.baseURI)).then(r=>r.json()));
  assert.equal(manifest.theme_color,'#ffffff','installed app navigation surface stays light');
