...
(async()=>{
 ...
 try{
  ...
  assert.equal(area.height,'24px');assert.equal(area.color,'rgb(247, 249, 251)');assert.equal(area.bottom,'36px');assert(area.scheme.includes('light')&&area.scheme.includes('only'));
