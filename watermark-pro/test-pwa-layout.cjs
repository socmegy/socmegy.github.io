...
(async()=>{
 ...
 try{
  ...
  assert.equal(area.height,'24px');assert.equal(area.color,'rgb(255, 255, 255)');assert.equal(area.bottom,'36px');assert(area.scheme.includes('light')&&area.scheme.includes('only'));
