...
module.exports=async({worker,env,db,req,admin})=>{
 ...
 try{
  ...
  await page.goto(origin+'/watermark-pro/index.html',{waitUntil:'domcontentloaded'});
  await page.waitForSelector('#wpSessionBoot',{state:'detached',timeout:20000});
  ...
  for(const selector of ['#downloadProgressImage','#downloadSupporterCard']){
   ...
   await page.waitForFunction(()=>{const toast=document.querySelector('#wpDownloadToast');return toast?.classList.contains('is-visible')&&/Muat turun dimulakan/i.test(toast.textContent||'')},null,{timeout:5000});
   if(selector==='#downloadSupporterCard')await file.saveAs('test-supporter-export.png');
  }
  /* Large cumulative values must remain contained on a phone-sized profile
     card; this mirrors the billion-sized values reported in the screenshot. */
  await page.setViewportSize({width:390,height:844});
  const progressLayout=await page.evaluate(async()=>{
   const values={totalDownloads:'1,123,703,383',imageDownloads:'559,806,932',videoDownloads:'563,896,451',nextMilestoneText:'Complete'};
   Object.entries(values).forEach(([id,value])=>{const el=document.getElementById(id);if(el)el.textContent=value});
   await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
   const shell=document.querySelector('.progress-shell'),rects=[...document.querySelectorAll('.progress-stats>div')].map(el=>{const r=el.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom}});
   return {overflow:(shell?.scrollWidth||0)-(shell?.clientWidth||0),shell:shell?.getBoundingClientRect().toJSON(),rects};
  });
  assert(progressLayout.overflow<=1,'profile progress overflows at billion counts '+JSON.stringify(progressLayout));
  assert(progressLayout.rects.every(r=>r.left>=0&&r.right<=390+1),'profile metric escapes viewport '+JSON.stringify(progressLayout));
  await page.setViewportSize({width:1366,height:900});
  ...
  const controlReceiptPrint=await control.evaluate(async()=>{
   ...
   const trigger=document.querySelector('[data-receipt-history], [data-receipt-submission]');
   ...
   try{window.printControlReceipt();await new Promise(resolve=>setTimeout(resolve,1000))}finally{window.open=originalOpen;document.getElementById('receiptModal')?.classList.remove('open')}
  ...
  const topDownload=control.waitForEvent('download');await control.locator('#downloadTopUsers').click();const topFile=await topDownload;
  assert.equal(await topFile.failure(),null);assert.match(topFile.suggestedFilename(),/\.png$/i);
