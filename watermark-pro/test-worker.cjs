...
// Deliberately omit read migration: account loading must recover an older DB.
...
{
 const noticeId=state.data.notifications[0].id;
 assert.equal((await req('/api/account/notifications/read','POST',{id:noticeId},token)).status,200);
 const second=await req('/api/auth/login','POST',{email:'test@example.invalid',password:'Test-password-2026'});
 assert.ok((await req('/api/account/state','GET',null,second.data.token)).data.notifications[0].readAt);
 await req('/api/account/notifications/read','POST',{id:noticeId},second.data.token);
 assert.equal(db.prepare('SELECT count(*) AS n FROM notification_reads').get().n,1);
 assert.equal((await req('/api/account/notifications/read','POST',{id:'missing'},token)).status,404);
